...
## M3 preparation and Azure acceptance

`CloudSecurityTests` exercises real Kestrel/JWT middleware with synthetic RSA keys and test-only issuer metadata: valid tokens, missing/forged/expired/wrong-issuer/audience tokens, and wrong tenant/caller/delegated/user claims. It also checks the shipping Production route guards, validated SQL configuration and receiver token destination restriction. This tests validation behavior without pretending to acquire a real Entra managed-identity token.

`SchemaDeploymentTests` uses disposable real SQL databases: fresh API/receiver baseline creation, repeat initialization, incompatible markers, and refusal of an unversioned M2 database while its accepted work remains. Azure `CREATE USER ... WITH SID ... TYPE=E`, managed-identity login and effective runtime grants require cloud verification.

`eng/verify-m3.ps1` performs Terraform formatting, backend-disabled locked provider initialization/validation, actionlint and PowerShell parsing before the unfiltered application acceptance and fresh Linux container demo. Windows/Linux provider lock hashes are retained. No Terraform plan, Azure provisioning or remote CI is implied by static checks.

`eng/cloud/verify.ps1` requires real deployed apps: unauthorized ingress, separate receiver workload/diagnostic ACLs, fresh idempotent acceptance, delivery/effect, retry exhaustion, idempotent replay and worker/receiver revision restart against retained Azure SQL. It records the actual pre-restart state and accepts honest Unknown history; Azure CLI restart does not establish the exact synchronized SIGKILL boundary of the local demo. Application Insights queries require real correlated lifecycle spans plus positive counters for all three services and worker histogram samples. Metrics are observed before replica replacement to avoid treating volatile exporter buffers as durable. Cloud rollback additionally requires a distinct previously verified compatible release, new revisions with its old digests and a passing fresh smoke. Teardown requires actual empty inventories. All cloud oracles are **unexecuted** until approved Azure execution.
